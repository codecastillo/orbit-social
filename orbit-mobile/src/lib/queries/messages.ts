...
export interface ConversationWithPreview {
  ...
  unread: boolean;
  /** Derived, never stored. See getRequestConversationIds. */
  is_request: boolean;
}
 ...
 * flipping accepts the thread, which is why Accept is simply a read. Mirrors
 * the web derivation in src/lib/queries/messages.ts.
 */
async function getRequestConversationIds(
  ...
  const repliedIn = new Set(
    (sent.data ?? []).map((m: { conversation_id: string }) => m.conversation_id),
  );
  const following = new Set(
    (follows.data ?? []).map((f: { following_id: string }) => f.following_id),
  );
...
export async function getConversations(
  ...
  const requestIds = await getRequestConversationIds(
    userId,
    visible
      .filter((c) => !c.is_group && c.created_by !== userId)
      .map((c) => c.id),
    otherMemberByConv,
    new Set(
...
export async function getWhoCanMessage(userId: string): Promise<WhoCanMessage> {
  ...
  return (
    ((data as { who_can_message?: WhoCanMessage }).who_can_message as
      | WhoCanMessage
      | undefined) ?? "everyone"
  );
}
