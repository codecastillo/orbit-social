...
/**
 * Keeps the signed-in user's last_seen_at fresh. Mount once, at the layout
 * level. Writes only while the tab is visible, at most once per HEARTBEAT_MS,
 * and never at all while the user hides their activity status.
 */
export function usePresenceHeartbeat() {
  ...
  useEffect(() => {
    ...
    const beat = () => {
      ...
      if (Date.now() - lastBeatAt < HEARTBEAT_MS) return;
      lastBeatAt = Date.now();
...
export function usePresence(targetId: string | null | undefined): Presence | null {
  const { user } = useAuth();
  const { data } = useQuery({
