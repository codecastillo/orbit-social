...
/** How often a presence display refreshes while a surface is open. */
 ...
 * contract read receipts use (see getDmSeenAt).
 */
export async function getHideActivity(userId: string): Promise<boolean> {
  ...
  return data.hide_activity ?? false;
}
...
export function presenceOf(lastSeenAt: string | null | undefined): Presence | null {
  ...
  if (age < HOUR_MS) {
    return { online: false, label: `Active ${Math.floor(age / MINUTE_MS)}m ago` };
  }
  if (age < DAY_MS) {
